...
DEFCONFIG=ASKernel-a7xeltekor_defconfig
...
CLEANUP()
{
	...
	rm -f "$KERNELDIR"/bootimg_tools/boot_a7xeltekor/kernel
	echo "Deleting all files from ramdisk dir in bootimg_tools if it exists"
	if [ ! -d "$KERNELDIR"/bootimg_tools/boot_a7xeltekor/ramdisk ]; then
		mkdir -p "$KERNELDIR"/bootimg_tools/boot_a7xeltekor/ramdisk 
		chmod 777 "$KERNELDIR"/bootimg_tools/boot_a7xeltekor/ramdisk
	else
		rm -rf "$KERNELDIR"/bootimg_tools/boot_a7xeltekor/ramdisk/*
	fi;
...
BUILD_NOW()
{
	...
	if [ -e "$KERNELDIR"/arch/arm64/boot/Image ]; then
		...
		cd bootimg_tools
		./mkboot boot_a7xeltekor ../../READY/boot_kor.img
		
		# Remove seandroid not enforcing warning on boot
		cd ../../READY
		echo "SEANDROIDENFORCE" >> boot_kor.img
